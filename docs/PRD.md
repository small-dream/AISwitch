...
| ID    | 用户故事                                                                      | 验收标准                                                                                                                |
| ----- | ----------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| US-01 | 作为用户，启动时自动**探测**本机 Claude Code / Codex 的安装状态与当前生效配置 | ① 三态：未安装（引导态）/ 已安装（显示当前供应商与模型）/ 未知（配置存在但无法解析，给出警告）；② 启动后 3 秒内完成探测 |
...
| US-07 | 从本机现有配置**一键导入**生成预设           | 解析当前 settings.json / config.toml 反填表单 |
...
| 工具        | 判定已安装                                             | 当前配置来源                    | 解析失败                  |
| ----------- | ------------------------------------------------------ | ------------------------------- | ------------------------- |
| Claude Code | 存在 `~/.claude/settings.json` 或 PATH 中存在 `claude` | `env` 字段各键                  | 状态置为 `unknown` 并警告 |
| Codex CLI   | 存在 `~/.codex/config.toml`                            | 顶层 `model` / `model_provider` | 状态置为 `unknown` 并警告 |

均不存在 → `not-installed`（展示引导态）。
