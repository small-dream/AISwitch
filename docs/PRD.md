...
- **检查与下载**：桌面端启动后静默检查一次；顶栏始终提供「检查更新」按钮，用户也可随时手动触发检查和预下载。检查中按钮禁用并显示进行状态；无新版本、检查/下载失败时，手动操作通过 Toast 提示结果。
- **用户确认**：更新包下载完成后，「检查更新」按钮切换为带版本号的「更新」按钮；用户点击后安装并重启应用，安装失败通过 Toast 提示且不隐藏原应用。
