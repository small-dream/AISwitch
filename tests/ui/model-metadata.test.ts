...
describe('parseModelMetadataField', () => {
  ...
  it('非法 JSON / 非对象 → 明确报错', () => {
    expect(parseModelMetadataField('{oops', 'm').error).toBe('不是合法 JSON')
    expect(parseModelMetadataField('[1, 2]', 'm').error).toContain('JSON 对象')
    expect(parseModelMetadataField('42', 'm').error).toContain('JSON 对象')
  })
  ...
  it('slug 与模型名不一致 → 报错并提示两端值', () => {
    const { error } = parseModelMetadataField('{"slug":"other"}', 'glm-4.6')
    expect(error).toContain('other')
    expect(error).toContain('glm-4.6')
  })
  ...
  it('粘贴整份 models.json → 原样保存（需含当前模型条目）；无匹配条目 → 报错', () => {
    ...
    expect(parseModelMetadataField(text, 'gpt-5.5').error).toContain('gpt-5.5')
  })
})
...
describe('presetFormSchema · modelMetadataJson 跨字段校验', () => {
  ...
  it('Codex：合法条目通过，slug 不一致挂在 modelMetadataJson 错误上', () => {
    expect(presetFormSchema.safeParse({ ...BASE, modelMetadataJson: '{"context_window":1}' }).success).toBe(true)
